...
const towStatus = async (req, res) => {
  ...
  try {
    ...
    if (isSessionOngoing) {
      ...
      const lastEndedSession = await Tow.findOne({
        licenseNumber: { $in: vehicles },
      })
        .sort({ "journey.startTime": -1 })
        .limit(1);
      ...
      currLocation = {
        latitude:
          lastEndedSession.journey[lastEndedSession.journey.length - 1].lat,
        longitude:
          lastEndedSession.journey[lastEndedSession.journey.length - 1].long,
      };
    }

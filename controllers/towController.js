...
const createTow = async (req, res) => {
	...
	try {
		const { policeId, vehicles, journey, endLocation, currentLocation, sessionEnd } = req.body;
		const linkId = generateRandomLinkId();

		if (!policeId || vehicles.length == 0) {
			return res.status(500).json({ message: 'Please specify all the fields', success: false });
		}
		...
		for (const licenseNumber of vehicles) {
			...
			if (!vehicle) {
                ...
                if (!vehicleLocation || !vehicleLocation.lat || !vehicleLocation.long) {
					return res.status(500).json({ message: `Invalid location data for vehicle ${licenseNumber}`, success: false });
                }
...
const updateTowById = async (req, res) => {
    try {
        ...
        for (const licenseNumber of vehicles) {
            ...
            if (!vehicle) {
                return res.status(500).json({ message: `Vehicle with license number ${licenseNumber} not found`, success: false  });
            }
...
const deleteTowById = async (req, res) => {
	try {
		const { id } = req.params;
		if (!id) {
			return res.status(500).json({ message: 'ID parameter is missing', success: false  });
		}
		const deletedTow = await Tow.findByIdAndDelete(id);
		if (!deletedTow) {
			return res.status(404).json({ message: 'Tow session not found', success: false  });
		}
		res.status(200).json({
			data: deletedTow,
			success: true,
...
const  towStatus = async (req, res) => {
	...
	try {
        ...
        if (isSessionOngoing) {
            ...
            if (!lastEndedSession) {
                return res.status(500).json({ message: 'No ended session found for the provided vehicle ID', success: false  });
            }
            currLocation = {
...
const deleteVehicle = async (req, res) => {
    try {
        ...
        if (!tow) {
            ...
            return res.status(404).json({ message: 'No tow found containing the specified vehicle.', success: false });
        }
        ...
        if (tow.sessionEnd === false) {
            ...
            const vehicleIndex = tow.licenseNumber.findIndex(license => license === vehicles);

            if (vehicleIndex === -1) {
                console.log('Vehicle not found in the Tow document.');
                return res.status(404).json({ message: 'Vehicle not found in the Tow document.', success: false });
            }
            ...
            if (!mongoose.Types.ObjectId.isValid(tow.policeId)) {
                ...
                return res.status(400).json({ message: 'Invalid or missing policeId.', success: false });
            }
            ...
            return res.status(500).json({ message: `No Live Session found of ${vehicles}`, success: false });
        }
    } catch (error) {
        console.error('Error deleting vehicle data:', error);
        return res.status(500).json({ message: 'Error deleting vehicle data', success: false });
    }
};
